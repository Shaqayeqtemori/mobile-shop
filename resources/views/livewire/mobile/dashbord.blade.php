...
<path d="M18.3401 20C19.0601 19.85 19.7401 19.56 20.3001 19.13C21.8601 17.96 21.8601 16.03 20.3001 14.86C19.7501 14.44 19.0801 14.16 18.3701 14" stroke="#FF00D4" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
 ...
 <div class="card-anim bg-white shadow-lg rounded-xl border-r-4 border-b-4 border-blue-500 p-2 "style="border-color: rgba(9, 72, 238, 0.25);">
                <div class=" flex items-end justify-between">
          <p class=" text-[12px] font-bold">مبایل های فروخته شده</p>
       <svg class="bg-blue-100 p-1 rounded-full" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_317_169)">
<path d="M7.49984 18.3332H12.4998C16.6665 18.3332 18.3332 16.6665 18.3332 12.4998V7.49984C18.3332 3.33317 16.6665 1.6665 12.4998 1.6665H7.49984C3.33317 1.6665 1.6665 3.33317 1.6665 7.49984V12.4998C1.6665 16.6665 3.33317 18.3332 7.49984 18.3332Z" stroke="#0099FF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M11.9593 9.09177C11.8759 9.68344 11.5259 10.1834 11.0426 10.4834V12.1334C11.0426 12.7084 10.5759 13.1751 10.0009 13.1751C9.42592 13.1751 8.95925 12.7084 8.95925 12.1334V10.4834C8.47592 10.1834 8.12591 9.68344 8.04258 9.09177C8.02591 9.0001 8.01758 8.9001 8.01758 8.8001C8.01758 7.53343 9.21759 6.5501 10.5343 6.9001C11.2009 7.0751 11.7426 7.61677 11.9176 8.28344C11.9926 8.55844 12.0009 8.83344 11.9593 9.09177Z" stroke="#0099FF" stroke-width="1.5" stroke-miterlimit="10"/>
<path d="M18.3345 9.0918H11.9595" stroke="#0099FF" stroke-width="1.5" stroke-miterlimit="10"/>
<path d="M8.0415 9.0918H1.6665" stroke="#0099FF" stroke-width="1.5" stroke-miterlimit="10"/>
</g>
<defs>
<clipPath id="clip0_317_169">
...
// =============== کمک‌تابع‌ها ===============
function setRingPercent(elId, percent){
  const el = document.getElementById(elId)
  const radius = Number(el.getAttribute('r'))
  const max = Math.PI * 2 * radius
  const filled = max * (percent/100)
  el.setAttribute('stroke-dasharray', `${filled} ${max}`)
}

function money(num){
  return new Intl.NumberFormat('fa-AF', { style: 'currency', currency: 'AFN', maximumFractionDigits: 0 }).format(num)
}

// =============== رندر حلقه‌ها ===============
setRingPercent('ring-new', rings.new)
setRingPercent('ring-second', rings.second)
setRingPercent('ring-returns', rings.returns)

// نمایش درصد روی کارت‌ها
document.getElementById('pct-new').textContent = `+${rings.new}%`
document.getElementById('pct-second').textContent = `+${rings.second}%`
document.getElementById('pct-returns').textContent = `+${rings.returns}%`

// =============== نمودارها ===============
function createGradient(ctx, color, alpha=0.15){
